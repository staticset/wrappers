...
- **Локальные функции в фильтрах нешипуемы** (`tool.split([**param])` —
  ...
  группирует локально (корректно, но не оптимально). Рецепты полного
  пушдауна (оба проверены на проде):
  ...
  - если значение контрола приходит `|||`-строкой и нужно распарсить в SQL —
    вместо SRF использовать **скалярную** форму
    `c.monthofyearid = ANY (string_to_array([**month], '|||')::int[])`:
    `string_to_array` immutable → планировщик фолдит её в константный массив
    `'{1,2,3}'::integer[]` ещё до пушдауна, postgres_fdw шипует весь запрос,
    а мост переводит `= ANY ('{…}'::type[])` в `IN (…)` (проверено 07.09:
    5-табличный LEFT JOIN + 4 таких фильтра + GROUP BY — один T-SQL).
    Каст `::int[]` обязателен (совпадает с типом колонки); для текстовых
    колонок — `::text[]`.
- **Оконные функции не пушатся вообще** (проверено 07.09): postgres_fdw не
  умеет шиповать `WindowAgg` (ни на верхнем уровне, ни внутри FROM-подзапроса)
  и агрегат над окном — `EXPLAIN` даёт `HashAggregate → WindowAgg → Foreign
  Scan`: в MSSQL уезжает весь join (postgres_fdw даже добавляет ORDER BY по
  PARTITION BY-ключам для локального окна), а окно и группировка считаются на
  Навигаторе. Это ограничение postgres_fdw (upstream), не моста; с 81eb6b8
  константные GROUP BY-items при этом не ломают удалённый запрос. Рецепт для
  конструктора: колонку с `SUM(...) OVER (PARTITION BY a, b)` при внешнем
  `GROUP BY a, b` можно заменить обычным агрегатом `SUM(x)/SUM(y)*100` —
  семантически то же самое и уезжает в MSSQL целиком.
